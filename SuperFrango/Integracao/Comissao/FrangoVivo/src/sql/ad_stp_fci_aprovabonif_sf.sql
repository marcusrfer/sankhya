...
begin
 ...
 if i > 0 then
  ...
  begin
   ...
   /*cfg.nuconf := ad_pkg_fci.get_config(b.numlote);
   
   select * into cfg from ad_tsffciconf where nuconf = cfg.nuconf;
   
   select codnat, codcencus
     into fin.codnat, fin.codcencus
     from tgfcab
    where nunota = cfg.nunotabnf;*/
   ...
   update ad_tsffci f set f.tipobonif = b.tipobonif where numlote = b.numlote;
