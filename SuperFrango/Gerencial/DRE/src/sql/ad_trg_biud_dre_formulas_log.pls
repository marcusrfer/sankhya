...
begin
  ...
  */
  ...
  if inserting then
    ...
    values
      (v_seqhist, 'INSERT', stp_get_codusulogado, sysdate, :new.codform, :new.tipoind, :new.query,
       :new.base);
  elsif updating then
    insert into dre_formulas_log
      (seqhist, operacao, codusu, dhalter, codform, tipoind, query, base)
    values
      (v_seqhist, 'UPDATE NEW VALUES', stp_get_codusulogado, sysdate, :new.codform, :new.tipoind,
       :new.query, :new.base);
  
    insert into dre_formulas_log
      (seqhist, operacao, codusu, dhalter, codform, tipoind, query, base)
    values
      (v_seqhist + 1, 'UPDATE OLD VALUES', stp_get_codusulogado, sysdate, :old.codform,
       :old.tipoind, :old.query, :old.base);
  elsif deleting then
