...
begin
 ...
 /*
  ...
  Objetivo: Calcular as diversas possibilidades de bonificações
 */
 ...
 if l.statusbonif in ('F', 'L') then
  p_mensagem := 'Bonificação já foi calculada e possui nota gerada, ' ||
 ...
 loop
  ...
  begin
   ...
   if p_tipobonif = 'M' then
    ...
    if p_mensagem is not null then
     stp_set_atualizando('N');
     p_mensagem := 'Erro ao calcular a bonificação da mortalidade. ' || p_mensagem;
     return;
     rollback;
    end if;
    ...
    ad_pkg_fci.set_bnf_carcaca(l.numlote, p_mensagem);
    if p_mensagem is not null then
     stp_set_atualizando('N');
     p_mensagem := 'Erro ao calcular a bonificação pela carcaça. ' || p_mensagem;
     return;
     rollback;
    end if;
    ...
    begin
     ...
     if nvl(p_vlrcusto, 0) = 0 then
      ...
      /*v_confirma := act_confirmar(p_titulo    => 'Confirmação de valores',
                                  p_texto     => 'O custo da ave não foi informada no parâmetro, o valor ' ||
                                                 ' do custo será o informado nas configurações da rotina (' ||
                                                 fmt.valor_moeda(p_vlrcusto) || '). <br>' ||
                                                 'Confirma a utilização desse valor?',
                                  p_chave     => p_idsessao,
                                  p_sequencia => 0);
     ...
     b.obs := 'Utilizando tabela ' || t.codtab || '/' || t.codemp || ', custo da ave de 0' ||
              fmt.numero(p_vlrcusto) || ' e com ajuda extra de ' || fmt.numero(p_vlrbnfpinto);
     ...
     values
      (l.numlote, i, 'ITA', c.percmortprev, l.qtdaves * (c.percmortprev / 100),
       ...
       b.vlrcom, b.vlrcom / l.qtdabat, b.vlrbonific, b.vlrunitbnf, 'N', b.obs);
    
    exception
     when others then
      rollback;
      p_mensagem := 'Erro ao inserir o cálculo da bonificação Itaberaí. ' || sqlerrm;
      return;
    end;
    ...
    begin
     ...
     b.vlrunitbnf := snk_dividir(b.vlrbonific, l.qtdabat);
     b.obs        := 'Utilizando tabela ' || t.codtab || '/' || t.codemp || ', custo da ave de 0' ||
                     fmt.numero(p_vlrcusto) || ' e com ajuda de ' || fmt.numero(p_vlrbnfpinto);
     ...
     values
      (l.numlote, i, 'NVZ', c.percmortprev, l.qtdaves * (c.percmortprev / 100),
       l.qtdaves - (l.qtdaves * (c.percmortprev / 100)), (l.qtdmortes / l.qtdaves) * 100,
       l.qtdmortes, l.qtdaves - l.qtdmortes, 0, 0, l.viabilidade, 100 - l.viabilidade, l.percom,
       b.vlrcom, b.vlrcom / l.qtdabat, b.vlrbonific, b.vlrunitbnf, 'N');
    end;
   end if;
 ...
 -- atualiza dados no formulario das bonificações
 begin
 ...
 exception
  when others then
   p_mensagem := 'Erro ao atualizar os dados na tela do cálculo da bonificação. ' || sqlerrm;
   return;
   rollback;
 end;
 ...
 begin
  ...
  if enviamail then
   ...
   mail.mensagem := 'Uma nova solicitação de aprovação de bonificação foi gerada ' ||
                    ...
                    fmt.numero(l.qtdaves) || ')' || ', por ' || ad_get.nomeusu(825, 'completo') ||
                    '.<br>' || chr(13) || 'Acesse o link abaixo para maiores detalhes.<br>' ||
                    chr(13) || '<a href="' || ad_fnc_urlskw('AD_TSFFCI', l.numlote) ||
                    '">Qlique Aqui</a>';
