create or replace package ad_pkg_func is
  ...
  function get_descr_estado_civil(p_estcivil int) return varchar2;

  function get_descr_escolaridade(p_codgrau int) return varchar2;
  ...
  function get_descricao_atributo(p_codemp number,
                                  p_codfunc number,
                                  p_atrib number) return varchar2;

  function get_cidade_uf(p_emp number,
                         p_mat number,
  ...
  function retorna_dados_periodo(p_dtref date,
                                 p_codemp number,
                                 p_matfunc number) return tab_dados_periodo
    pipelined;

  function get_descr_lotacao(p_codlot number,
                             p_grau int) return varchar2;

  /* Retorna a descrição do funcionário em determinado período*/
  function get_descrformacao_func(p_codemp int,
                                  p_matfunc number,
                                  p_dtref date) return varchar2;

end ad_pkg_func;
/
create or replace package body ad_pkg_func is

  function get_descr_estado_civil(p_estcivil int) return varchar2 is
  ...
  function get_descr_escolaridade(p_codgrau int) return varchar2 is
  ...
  function retorna_dados_periodo(p_dtref date,
    ...
    t rec_dados_periodo;
  begin
    select codemp, matfunc, codsit, grauinstr, codlot, codcargo
      ...
      from (with maior_data as (select f.fuhistcodemp codemp,
                                 ...
                                 where p_dtref between trunc(fmt.number_to_date(f.fuhistdataini), 'fmmm') and
  ...
  function get_descricao_atributo(p_codemp number,
                                  p_codfunc number,
                                  p_atrib number) return varchar2 is
  ...
  begin
    ...
    select case
      ...
      into v_nomecid, v_uf
      from (select nvl(trim(translate(replace(a.afvalor, ' - ', '/'), '1234567890', '          ')), m.mudesmunic) v_string,
  ...
  begin
    ...
    select lodesclot into v_result from fpwpower.lotacoes l where l.locodlot = v_codlot;
  ...
  function get_descrformacao_func(p_codemp int,
                                  p_matfunc number,
                                  p_dtref date) return varchar2 is
    v_result varchar2(4000);
  begin
