create or replace trigger ad_trg_cmp_adtssapar_sf
 for insert or update or delete on ad_adtssapar
 compound trigger
 ...
 **************************/
 ...
 begin
  ...
  exception
   when no_data_found then
    raise_application_error(-20105,
                            ad_fnc_formataerro('Usuário não possui permissão para utilizar esse processo!'));
  end;
  ...
  if updating then
     ...
     if (:new.dtvenc - :new.dtvencinic) > 0 and (:new.dtvenc - :new.dtvencinic) > v_ajustavenc then
      raise_application_error(-20101,
                              fc_formatahtml_sf('Alteração Cancelada',
                                                'Ajuste vencimento superior ao permitido',
                                                null));
     end if;
    end if;
   end if;
   ...
   if updating('VLRDESDOB') then
    ...
    if v_altvlrdesdob = 'N' and (:new.vlrdesdob <> :old.vlrdesdob) then
     raise_application_error(-20105,
                             ad_fnc_formataerro('Alteração Cancelada!<br>Usuário logado não tem permissões ' ||
  ...
  if deleting then
   ...
   if not ad_pkg_var.permite_update then
    raise_application_error(-20101,
                            fc_formatahtml_sf('Exclusão não permitida, as parcelas são geradas ' ||
