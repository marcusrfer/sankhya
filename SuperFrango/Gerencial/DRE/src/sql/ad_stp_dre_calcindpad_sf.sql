create or replace procedure ad_stp_dre_calcindpad_sf(p_codusu    number,
                                                     p_idsessao  varchar2,
                                                     p_qtdlinhas number,
  ...
  */
  ...
  begin
    ...
    loop
      ...
      ad_pkg_newdre.set_rentabcom(p_referencia, rel(z), p_mensagem);
      ad_pkg_newdre.set_resindpad(p_referencia, rel(z), p_mensagem);
  ...
  ft := trunc((dbms_utility.get_time - it) / 100);

  v_time := to_char(to_date(ft, 'sssss'), 'hh24:mi:ss');

  p_mensagem := 'Indicadores calculados com SUCESSO!!!<br> Dependentes recalculados:<br>' || v_mensagem ||
                '.<br> Temp de execução: ' || v_time || '.';
