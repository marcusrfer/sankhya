...
begin
 ...
 begin
  if nvl(conf.renovar, 'N') = 'S' then
   ...
   select nvl(sum(p.vlrdesdob), 0) into v_totdespren from ad_adtssaparrenovar p where p.nunico = cab.nunico;
 ...
 begin
  ...
  if (v_solicitajuro = 1 or v_solicitaparcela = 1 or v_solicitavalor = 1 or v_solicitacarencia = 1) and
   ...
   if v_solicitajuro = 1 then
    v_mensagemusu := 'Juro Minimo: ' || nvl(conf.juro, 0) || '% - Juro Negociado: ' || nvl(cab.taxa, 0) ||
                     '%. ';
   end if;
   ...
   if v_solicitaparcela = 1 then
    v_mensagemusu := case
                      when v_mensagemusu is not null then
                       v_mensagemusu || '\nNr. Máximo Parcelas: ' || conf.parcela || ' Nr. Parcelas Negociado: ' || cab.nrparcelas || '. '
                      else
                       v_mensagemusu || 'Nr. Máximo Parcelas: ' || conf.parcela || ' Nr. Parcelas Negociado: ' || cab.nrparcelas || '. '
                     end;
   end if;
   ...
   if v_solicitavalor = 1 then
    v_mensagemusu := case
                      when v_mensagemusu is not null then
                       v_mensagemusu || '\nVlr Máx Configurado: ' || ad_get.formatavalor(conf.vlrmax) || ' Vlr Negociado: ' ||
                       ad_get.formatavalor(cab.vlrdesdob) || '. '
                      else
                       v_mensagemusu || 'Vlr Máx Configurado: ' || ad_get.formatavalor(conf.vlrmax) || ' Vlr Negociado: ' ||
                       ad_get.formatavalor(cab.vlrdesdob) || '. '
                     end;
   end if;
