Create Or Replace Procedure "AD_STP_DEF_GERAPEDCPA"(p_Codusu    Number,
																										p_Idsessao  Varchar2,
																										p_Qtdlinhas Number,
...
Begin
  ...
  Objtivo: Gerar um pedido de compras para o fornecedor vencedor da cotação ou atualizar o pedido d compras de origem
  ...
  ******************************************************************************************************************/
	...
	Loop
		...
		If r_Cot.Situacao Not In ('C', 'L') Then
			Errmsg := 'Somente cotações concluídas/liberadas podem gerar pedidos.';
			Raise error;
		End If;
	
		If r_Cot.Nunota Is Null Then
			--errmsg := 'Cotação já possui pedido de compra gerado. Nro. Único: ' || r_Cot.Nunota;
			--Raise Error;
			v_Pedido := True;
		End If;
		...
		If v_Pedido Then
			...
			Exception
				When No_Data_Found Then
					Errmsg := 'Não foi informada nenhuma TOP para gerar o pedido de compras';
					Raise Error;
			End;
			...
			ad_set.Ins_Pedidocab(p_Codemp      => r_Cot.codemp,
													 p_Codparc     => r_Forn.Codparc,
													 p_Codvend     => v_CodVend,
													 p_Codtipoper  => r_Cot.Codtipoper,
													 p_Codtipvenda => r_Cot.Codtipvenda,
													 p_Dtneg       => Trunc(Sysdate),
													 p_Vlrnota     => r_Forn.Vlrtot,
													 p_Codnat      => r_cot.codnat,
													 p_Codcencus   => r_Cot.Codcencus,
													 p_codproj     => 0,
													 p_Obs         => 'Cotação: ' || To_Char(r_Cot.Numcotacao),
													 p_Nunota      => v_Nunota);
      ...
      Exception
        When Others Then
          Errmsg := 'Erro ao inserir o cabeçalho do pedido. ' || chr(13) || Sqlerrm;
          Raise error;
      End;*/
			...
			Loop
				...
				ad_set.ins_pedidoitens(v_Nunota,
															 c_Prod.Codprod,
															 c_prod.qtdneg,
															 c_Prod.Vlrunit,
															 c_Prod.Vlrtotal,
				...
				Exception
					When Others Then
						Errmsg := 'Erro ao atualizar a solicitação. ' || chr(13) || Sqlerrm;
						Raise error;
				End;
			...
			Exception
				When Others Then
					errmsg := 'Erro ao atualizar os totais do pedido. ' || chr(13) || Sqlerrm;
					Raise error;
			End;
			...
			If r_Top.Atualfin <> 0 Then
				...
				Loop
					...
					Begin
						...
						Values
							(v_Nufin,
							 ...
							 r_Cot.Codproj,
							 c_Tpv.Codtiptitpad,
							 (v_Vlrnota * (c_Tpv.Percentual / 100)),
							 r_Top.Atualfin,
							 v_Provisao,
							 'E',
							 v_Nunota,
					...
					Exception
						When Others Then
							Errmsg := 'Erro na inclusão das parcelas do financeiro. ' || chr(13) || Sqlerrm;
							Raise error;
					End;
			...
			Exception
				When Others Then
					Errmsg := 'Erro ao atualizar numeração. ' || chr(13) || Sqlerrm;
					Raise error;
			End;
		...
		Else
			...
			If v_Count != 0 Then
				Errmsg := 'Atualização de valores já realizada.';
				Raise error;
			End If;
			...
			Update tgfcab c
				 Set c.vlrfrete      = r_Forn.Vlrtot,
						 c.vlrfretetotal = r_Forn.Vlrtot,
						 c.codparctransp = r_forn.codparc,
						 c.tipfrete      = r_Forn.Tipfrete,
						 c.ad_cotfrete   = 'N',
						 c.cif_fob       = r_Forn.Cif_Fob,
						 c.vencfrete     = Sysdate + 5
			 Where c.nunota = r_Cot.Nunota;
			...
			Loop
				Update ad_tsfssti si
					 Set si.vlrunit = c_Itens.Vlrtotal,
							 si.qtdneg  = c_Itens.Qtdneg,
