		...
		If v.nometaborig = 'TCSCON' Then
			...
			Begin
				...
				If v_parcela <> 0 Or v_parcela Is Not Null Then
					Update tcscon Set parcelaatual = parcelaatual - 1 Where numcontrato = v.nuchaveorig;
				End If;
			End;
		End If;
		...
		If v.nometaborig = 'TCSCON' Then
			...
			Exception
				When Others Then
					...
					Values
						(Stp_get_codusulogado(), Sysdate, 'Erro ao desfazer a compensação do desconto no contrato.',
						 Ad_get.Nomemaquina(), Null);
			End;
		End If;
