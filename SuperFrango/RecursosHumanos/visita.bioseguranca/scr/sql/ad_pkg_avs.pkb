create or replace package body ad_pkg_avs is
  ...
  */
  ...
  begin
    ...
    select nvl(max(nuavshis), 0) + 1
      into i
      from ad_tsfavshis h
     where h.nuvisita = p_nuvisita;
  ...
  exception
    when others then
      raise_application_error(-20105,
                              ad_fnc_formataerro('Erro ao inserir histórico. ' ||
                                                 sqlerrm));
  end insere_historico;

  procedure insere_historico(p_nuvisita number,
                             p_msg      varchar2,
  ...
  */
  ...
  begin
    ...
    for v in (select vis.nomevisitado || ' - ' || car.descrcargo || '  - ' ||
                      lot.descrlot || chr(13) as complemento
                from ad_tsfavs vis
    ...
    begin
      ...
      if x = 0 then
        begin
          select lig.unidade
            into vis.coduni
            from fpw_lotacoes_lig lig
           where lig.codlot = fun.fucodlot;
        exception
      ...
      if nvl(vis.codend, 0) = 0 or nvl(vis.codbai, 0) = 0 then
        vis.complemento := fun.fuendereco || '  - ' || fun.fubairro || ' nro ' ||
                           fun.funumero || '  - ' || fun.fucomplemento;
      end if;
      ...
      vis.codcid       := ad_pkg_func.get_codcid_coduf(fun.fucodemp,
                                                       fun.fumatfunc,
                                                       'codcid');
      vis.coduf        := ad_pkg_func.get_codcid_coduf(fun.fucodemp,
                                                       fun.fumatfunc,
                                                       'coduf');
      ...
      begin
        ad_pkg_avs.insere_historico(vis.nuvisita,
                                    'Inserido por ' ||
                                    ad_get.nomeusu(v_codusu, 'completo'));
      exception
        when others then
          p_mensagem := sqlerrm;
          return;
  ...
  begin
    ...
    exception
      when others then
        ad_pkg_var.errmsg := 'Erro ao atualizar reagendamento da visita. ' ||
                             sqlerrm;
        raise_application_error(-20105, ad_fnc_formataerro(ad_pkg_var.errmsg));
    end;
  
    insere_historico(p_nuvisita,
                     'Reagendamento da visita para dia ' ||
                     to_char(p_newdata, 'dd/mm/yyyy'));
  ...
  procedure set_nova_pesquisa(p_nuvisita number,
                              p_pesquisa out number,
                              p_errmsg   out varchar2) is
  begin
  ...
  begin
    ...
    pes.observacao := pes.observacao || chr(13) || 'Tipo de visita: ' ||
                      ad_get.opcoescampo(vis.tipovisita,
                                         'TIPOVISITA',
                                         'AD_TSFAVS') || chr(13) || 'Obs: ' ||
                      vis.obs;
    ...
    for crg in (select *
                 ...
                 where c.codcargo = vis.codcargo
                   and c.codemp = 1)
    loop
      pes.observacao := pes.observacao || chr(13) || 'Cargo: ' ||
                        crg.descrcargo;
    end loop;
    ...
    loop
      pes.observacao := pes.observacao || chr(13) || 'Lotação: ' ||
                        lot.descrlot;
    end loop;
    ...
    loop
      pes.observacao := pes.observacao || chr(13) || 'Unidade: ' ||
                        ung.descrlot;
    end loop;
  
    -- insere pesquisa
    stp_keygen_tgfnum('AD_TSFPES',
                      1,
                      'AD_TSFPES',
                      'CODPESQUISA',
                      0,
  ...
  begin
    ...
    begin
      ...
      if p_solmotivo is null then
        sol.motivo := sol.motivo || chr(13) || 'Visita Sanitária nro: ' ||
                      vis.nuvisita || ' - ' || vis.nomevisitado;
      else
      ...
      stp_keygen_tgfnum('AD_TSFCAPSOL',
                        1,
                        'AD_TSFCAPSOL',
                        'NUCAPSOL',
                        0,
    ...
    begin
      ...
      ad_stp_cap_enviaagend(stp_get_codusulogado,
                            'NOVASOLICITACAOCARRO',
                            1,
                            p_errmsg);
