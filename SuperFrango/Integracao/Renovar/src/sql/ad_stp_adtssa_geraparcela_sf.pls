  ...
  ad_stp_adtssa_valida_cab_sf(p_nunico => cab.nunico,
                              p_val_total => case
                                                when nvl(conf.renovar, 'N') = 'S' then
                                                 ...
                                                 'S'
                                              end, p_solcarencia => v_solcarencia,
                              p_solvalor => v_solvalor, p_soljuro => v_soljuro,
                              p_solparc => v_solparc, p_mensagem => p_mensagem);
  ...
  begin
    ...
    if conf.renovar = 'S' then
      ...
      insert into ad_adtssapar
        (nunico, sequencia, nufin, dtvenc, vlrdesdob, vlrjuros, vlrtotal, recdesp, provisao,
         nrparcela, dtvencinic)
      values
        (cab.nunico, v_sequencia, null, cab.dtvenc, cab.vlrdesdob, 0, cab.vlrdesdob, v_recdesp, 'S',
         1, cab.dtvenc);
    ...
    loop
      ...
      p_vlr_jur_parc := case
                          when nvl(cab.taxa, 0) > 0 then
                           ad_get.calculajuroprice(i => cab.taxa, n => cab.nrparcelas,
                                                   pv => cab.vlrdesdob, p_dtneg => cab.dtvenc,
                                                   p_dtprimvenc => cab.dtvenc1, p_parcela => v_nrparcela,
  ...
  if v_dif <> 0 then
    begin
      update ad_adtssapar p
         set p.vlrdesdob = p.vlrdesdob + v_dif, p.vlrtotal = p.vlrtotal + v_dif
       where p.sequencia = 2
