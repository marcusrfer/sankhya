...
begin
  ...
  */
  ...
  cab.codemp := act_int_param(p_idsessao, 'CODEMP');
  ...
  loop
    ...
    begin
      select /*e.codemp, */
       u.codcencus, n.codnat, t.codtipoper, v.codtipvenda
    ...
    exception
      when others then
        p_mensagem := 'Erro ao consultar os parâmetros na Central de Parâmetros, regra 7. ' ||
                      'Verifique se a mesma existe.';
        return;
    end;
  
    -- alteração para buscar empresa preferencial do parceiro, atender empresas diferentes  
    begin
    ...
    exception
      when others then
        errm := ad_fnc_formataerro('Erro ao buscar empresa preferencial do parceiro. ' || sqlerrm);
        raise_application_error(-20105, errm);
    end;
