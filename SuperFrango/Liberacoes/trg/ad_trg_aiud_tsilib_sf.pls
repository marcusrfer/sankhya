 ...
 ** Atualizações **
 ...
 Descritivo: 001 - Dispara e-mail quando libera TOP 1009, se evento 1001 manda email para quem libera 1007, se evento 1007 dispara email para quem libera 1006, caso o usuário liberador não tenha tentado confirmar a nota
 */
 ...
 if inserting then
  ...
  if :new.dhlib is null then
   ...
   mail.mensagem := '<font align="left">Atenção, foi inserida uma nova Solicitação de Liberação para ' ||
                    ...
                    ad_get.nomeusu(:new.codususolicit, 'resumido') || '<br> <b>Número Único: </b>' ||
  ...
  if :new.tabela = 'AD_TSFDEF' then
   ...
   select nvl(u.nomeusucplt, u.nomeusu) into v_nomesolicit from tsiusu u where codusu = :new.codususolicit;
  ...
  */
  ...
  --    1051 - Aceite do RH para  solicitação de Benefício   
  --    (Evento disparado para que o RH possa Aceitar o registro de benefícios.)
  -------------------------------------------------------------------------
 
  if :new.tabela = substr('AD_BENEFICIOS', 0, 12) and :new.evento = 1051 then
  
   select nvl(u.nomeusucplt, u.nomeusu) into v_nomesolicit from tsiusu u where codusu = :new.codususolicit;
  
   mail.assunto := 'Nova solicitação de liberação.';
  
   mail.mensagem := '<br> Uma nova solicitação de Aceite de Benefícios , ' ||
                    'referente ao Código de Benefício  nº ' || :new.nuchave || ', no valor de ' ||
                    ad_get.formatavalor(:new.vlratual) || ', foi cadastrada no sistema por ' || v_nomesolicit;
  ...
  if :new.tabela = substr('AD_BENEFICIOS', 0, 12) and :new.evento = 1052 then
   ...
   mail.mensagem := '<br> Uma nova solicitação de Aprovação de Benefícios , ' ||
                    'referente ao Código de Benefício  nº ' || :new.nuchave || ', no valor de ' ||
                    ad_get.formatavalor(:new.vlratual) || ', foi cadastrada no sistema por ' || v_nomesolicit;
   v_enviamail   := 'S';
  end if;
  ...
  */
  ...
  if :new.codusulib > 0 then
   declare
    v_codfila number;
   begin
    stp_keygen_tgfnum(p_arquivo => 'AD_FILANOTIFICACAOPUSH',
                      p_codemp  => 1,
                      p_tabela  => 'AD_FILANOTIFICACAOPUSH',
                      p_campo   => 'CODFILA',
                      p_dsync   => 0,
                      p_ultcod  => v_codfila);
  ...
  if updating('DHLIB') and (:new.dhlib is not null and :old.dhlib is null) then
   ...
   if nvl(:new.reprovado, 'N') = 'N' then
    ...
    mail.mensagem := '<br> A solicitação de Liberação para ' || v_descrevento || ', ' ||
                     'referente ao código nº ' || :new.nuchave || ', foi liberada no sistema. linha 206';
   else
   ...
   if :new.tabela = 'TGFCAB' and :new.evento in (1001, 1007) then
    ...
    if v_count = 0 then
     ...
     mail.mensagem := '<br> A solicitação de ' || case
                      ...
                      ad_get.nomeusu(:new.codusulib, 'RESUMIDO') || ' no lançamento ' || :new.nuchave ||
   ...
   --  1051 - Aceite do RH para  solicitação de Benefício   (Evento disparado para que o RH possa Aceitar o registro de benefícios.)
   --  1052 - Aprovação do RH para Benefício Negociado (Evento disparado para que o RH possa Aprovar a negociação do benefícios.)
   -------------------------------------------------------------------------
   ...
   if :new.codusulib > 0 and mail.assunto is not null and v_enviamail = 'S' then
    ...
    ad_set.ins_avisosistema(p_titulo     => 'Liberação realizada.',
                            ...
                            p_solucao    => 'Verifique o evento ' || v_descrevento || ', lançamento ' ||
                                            :new.nuchave,
 ...
 if deleting then
  ...
  select nvl(u.nomeusucplt, u.nomeusu) into v_nomesolicit from tsiusu u where codusu = stp_get_codusulogado;
 
  mail.assunto  := 'Exclusão de liberação.';
  mail.mensagem := '<br> A solicitação de liberação para ' || v_descrevento || ', ' ||
                   'referente ao código nº ' || :old.nuchave || ', foi excluída no sistema por ' ||
                   v_nomesolicit;
