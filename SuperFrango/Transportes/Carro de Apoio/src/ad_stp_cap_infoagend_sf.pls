  ...
  loop
    ...
    begin
      select c.*
        ...
        join ad_tsfcap c
          on s.nuap = c.nuap
       where s.nucapsol = field_nucapsol;
    
      p_mensagem := 'O agendamento encontra-se <b>' ||
                    ad_get.opcoescampo(r_cap.status, 'STATUS', 'AD_TSFCAP') || '</b>.<br>';
      ...
      p_mensagem := p_mensagem || chr(13) || '<b>Veículo: </b>' ||
                    ad_get.formataplaca(r_cap.codveiculo) || '<br><br>';
