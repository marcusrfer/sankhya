create or replace procedure ad_stp_ndre_criabaseindpad(p_codusu    number,
                                                       p_idsessao  varchar2,
                                                       p_qtdlinhas number,
  ...
  */
  ...
  if ad_pkg_var.errmsg is null then
    p_mensagem := '<img src="http://chittagongit.com/images/green-check-mark-icon-png/green-check-mark-icon-png-13.jpg" ' ||
                  ...
                  '<p style="text-align:center;padding:10px 3px">' || '
		Base de dados recriada com sucesso' || ' em ' || trunc(ad_pkg_var.end_time) ||
                  ' Mins.</p>';
  else
