Create Or Replace Procedure "AD_STP_GERACPA"(P_CODUSU    Number,
																						 P_IDSESSAO  Varchar2,
																						 P_QTDLINHAS Number,
  ...
  */
	...
	Loop
		...
		Elsif r_def.status = 'P' Then
			ErrMsg := 'Já existe pedido de compras gerado para esse lançamento. Nro Único ' ||
								r_def.nunota;
			Raise error;
		End If;
		...
		Begin
			...
			Select Max(dhalter)
				Into v_maxDhTipvda
				From tgftpv v
			 Where v.codtipvenda = r_def.codtipvendaped;
		
			ad_get.get_next_cod_tgfnum(p_tabela  => 'TGFCAB',
																 p_codemp  => 1,
																 p_serie   => 'X',
																 p_proxcod => v_Nunota);
			...
			Values
				(v_Nunota,
				 ...
				 v_CodUsuLog,
				 r_top.tipmov,
				 'Ref.: despesas extras da Ordem de Serviço Nº ' || r_def.ordemcarga);
		Exception
			When Others Then
    ...
    */
    ...
    End;*/
		...
		Begin
			...
			Ad_set.Ins_Pedidoitens(p_Nunota   => v_Nunota,
														 p_Codprod  => r_def.codprod,
														 p_Qtdneg   => 1,
														 p_codvol   => r_prod.codvol,
														 p_Vlrunit  => v_VlrTotRec,
														 p_Vlrtotal => v_VlrTotRec,
														 p_Mensagem => errmsg);
			If ErrMsg Is Not Null Then
				Raise error;
			End If;
		...
		Begin
			...
			Loop
				...
				Begin
					For Rec In (Select Distinct m.descricao
												From ad_tsfdefr r
				...
				Begin
					ad_set.ins_financeiro(p_codemp     => r_def.codemp,
																p_numnota    => 0,
																p_dtneg      => Trunc(Sysdate),
																p_dtvenc     => v_dtvenc,
																p_codparc    => r_def.codparctransp,
																p_top        => r_def.codtipoperped,
																p_contabanco => 0,
																p_codnat     => v_CodNat,
																p_codcencus  => r_def.codcencusped,
																p_codproj    => 0,
																p_codtiptit  => 0,
																p_origem     => 'E',
																p_nunota     => v_Nunota,
																p_valor      => v_vlrParcela,
																p_Nufin      => v_Nufin);
				
					Update tgffin
						 Set historico = v_Historico
					 Where nufin = v_nufin;
				...
				Begin
					...
					Loop
						If Nvl(c_rec.rateio_oc, 'N') = 'S' Then
							...
							For c_Cus In (Select Distinct u.codcencus, Sum(c.vlrnota) valor
							...
							Loop
								...
								-- verifica se já existe rateio para esse CR
								Select Count(*)
									Into v_Count
									From tgfrat r
								 Where r.nufin = v_Nufin
									 And r.codnat = v_CodNat
									 And r.codcencus = c_cus.codcencus;
								...
								If v_count = 0 Then
									...
									Exception
										When Others Then
											ErrMsg := 'Erro ao inserir o rateio. ' || Sqlerrm;
											Raise error;
									End;
								Else
									-- se já existe
						...
						Else
							...
							Select Count(*)
							 ...
							 Where r.nufin = v_Nufin
								 And r.codnat = v_CodNat
								 And r.codcencus = c_rec.codcencus;
	...
	P_MENSAGEM := 'Pedido Nº único <a target="_parent" href="' || ad_fnc_urlskw('TGFCAB', v_nunota) ||
								'"> <u>' || v_nunota || '</u></a> inserido com sucesso.';
