      ...
      begin
        insert into ad_tsfcapexc
          (nuap, codususol, dhsolicit, ordemcarga, codusuexc, codparctransp, codveiculo, status,
           ...
           codciddest, codcidorig, nomecidorig, nomeciddest, dhmov, dtreabre, codusureabre,
           motivoreabre, nuappai, dhexclusao, codusudel)
        values
          (:old.nuap, :old.codususol, :old.dhsolicit, :old.ordemcarga, :old.codusuexc,
           :old.codparctransp, :old.codveiculo, :old.status, :old.taxi, :old.motivotaxi,
           :old.kminicial, :old.kmfinal, :old.totalkm, :old.vlrcorrida, :old.nucapsol, :old.dtagend,
           :old.rota, :old.dtagendfim, :old.combinada, :old.codcontato, :old.qtdpassageiros,
           :old.motorista, :old.motivo, :old.deptosol, :old.codciddest, :old.codcidorig,
           :old.nomecidorig, :old.nomeciddest, :old.dhmov, :old.dtreabre, :old.codusureabre,
           :old.motivoreabre, :old.nuappai, sysdate, stp_get_codusulogado);
      exception
        when others then
          ad_set.insere_msglog(p_mensagem => 'Erro ao gravar log de exclusão de agendamento de ' ||
                                             'carro de apoio. ' || sqlerrm);
      end;
    end if;
