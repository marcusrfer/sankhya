create or replace trigger trg_u_tsilib_provisao_sf
  ...
  /*
   ...
   AUTOR:        Por Ricardo em 17/12/2019 
   Objetivo:     10) Quando esta fazendo uma devolução por divergência dentro do Sankhyaw, precisei colocar na Trg_Cmp_Tgfcab_Confirma_Sf a TSILIB aprovada, quando na realidade
                 ela não foi aprovada, mas o W me exigiu isso. Por ter colocado ela como aprovada agora estou alterando novamente a TSILIB. Isso tudo foi necessário para que 
                 o W conseguisse gerar a nota de devolução, que não era gerada porque havia uma pendencia na TSILIB.
                 A Variaveis_Teto.v_Desfazer_Tsilib será acionada como true na Trg_Cmp_Tgfcab_Confirma_Sf
  ...
  */
  ...
  begin
    ...
    */
    ...
    if updating('DHLIB') and :new.dhlib is not null and
       :new.tabela in ('AD_ADTSSACAB', 'AD_ADTOCAB') then
    ...
    elsif updating('DHLIB') and :new.dhlib is not null and
          :new.evento in (1015, 1028, 1035 /*, 1042*/) then
    ...
    elsif updating('DHLIB') and :new.dhlib is not null and :new.evento in (1010) and
          :new.tabela = 'TGFFIN' then
        ...
        update tgffin f
           set f.provisao = 'N',
               ...
               f.historico = decode(v_forma,
                                    ...
                                    ' - ' || ad_get.nomeusu(:new.codusulib, 'Resumido') || ' / ' ||
                                    f.historico,
                                    'B',
  ...
  begin
    ...
    */
    if nvl(p_executar, false) then
      ...
      if r_lib.tabela = 'AD_ADTSSACAB' then
        ...
        if r_lib.reprovado = 'S' then
          ...
          if v_count > 0 and v_dhlib is null then
            raise_application_error(-20101,
                                    fc_formatahtml_sf('Ação Cancelada',
                                                      'Aguardando liberação responsável CR',
                                                      null));
      ...
      if r_lib.evento = 1035 and r_lib.tabela = 'TGFCAB' and
         r_lib.nuchave not in (22819315, 22807310, 30627426) then
        ...
        if r_lib.seqcascata = 2 then
          ...
          if v_count = 0 then
            ...
            if v_junior > 0 and nvl(r_lib.codtipoper, 0) > 0 then
              -- Por Ricardo Soares em 02/12/2019. Se TOP não exige aprovação do comprador então não tenho que avaliar essa situação, logo v_junior passa a ser 0
              -- O cenário descrito foi identificado em uma aprovação que esta sendo feita pelo Robson em uma nota fiscal de energia TGFCAB.NUNOTA 35566680
              select count(*)
                into v_count
                from tgftop t
               where t.codtipoper = r_lib.codtipoper
                 and t.dhalter = ad_get.maxdhtipoper(r_lib.codtipoper)
                 and t.ad_exigaprvde in ('A' /*TOP Exige Aprovação p/ Confirmar*/,
                                         'O' /*Somente da TOP (Lib. Cascata)*/,
                                         'R'
                                         /*Somente Responsável CR*/,
                                         'T' /*Liberadores do Contrato*/,
                                         'F' /*Top Financeira*/);
