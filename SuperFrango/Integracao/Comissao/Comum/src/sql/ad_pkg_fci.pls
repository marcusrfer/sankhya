CREATE OR REPLACE package ad_pkg_fci is
  ...
  procedure get_dados_tabela(p_nrolote number, p_codemp number, p_tabela out ad_tsftci%rowtype);
  ...
  procedure set_bnf_carcaca(p_nrolote number, p_vlrmedcom float, p_errmsg out varchar2);
...
CREATE OR REPLACE package body ad_pkg_fci is
  ...
  begin
    ...
    select p_nrolote, null, c.codemp, c.nunota, null, c.numnota, c.serienota, c.dtneg, c.dtfatur,
           c.vlrnota, c.codtipoper, c.tipmov, c.statusnota, c.statusnfe, i.qtdneg
      bulk collect
      into notas
      from tgfcab c
      join tgfite i
  ...
  begin
    ...
    select trunc(l.dhpega) into lote.datafim from ad_tsflfv l where l.numlote = lote.numlote;
  
    --dbms_output.put_line('Período: ' || lote.dataini || '  - ' || lote.datafim);
  
    select p_nrolote, null, c.codemp, c.nunota, null, c.numnota, c.serienota, c.dtneg, c.dtfatur,
           c.vlrnota, c.codtipoper, c.tipmov, c.statusnota, c.statusnfe,
           i.qtdneg * case
             when c.codtipoper = 331 then
              1
             else
              -1
           end qtdneg
     ...
     where c.codparc = lote.codparc
       ...
       and c.dtfatur >= lote.dataini - (8 / 30)
  ...
  begin
    ...
    select p_nrolote, null, f.codemp, null, f.nufin, f.numnota, f.serienota, f.dtneg, f.dtvenc,
           f.vlrdesdob, f.codtipoper, 'I', null, null, 1
      bulk collect
      into notas
      from tgffin f
  ...
  begin
    ...
    select trunc(l.dhpega) into lote.datafim from ad_tsflfv l where numlote = lote.numlote;
  ...
  procedure get_dados_tabela(p_nrolote number, p_codemp number, p_tabela out ad_tsftci%rowtype) as
    lote lote_ave%rowtype;
  begin
    lote := get_dados_lote(p_nrolote);
    ...
    exception
      when no_data_found then
        raise_application_error(-20105,
                                'Erro! Não há parametrização válida nessa ' || 'referência.<br>');
    end;
  end get_config;
  ...
  begin
    ...
    for dados in (
                  ...
                  select distinct lt.codparc, l.codprod, lt.qtdavesaloj qtdaves, l.descrabrevave,
                                   l.dtalojamento, l.qtdmortes, l.pesofinal, l.dtabate,
                                   to_date(l.dhpega, 'dd/mm/yyyy') dhpega, l.numlote, fv.idade, fv.sexo,
                                   lt.status, fv.codlote
                    from ad_tsflfv l
                    join lote_ave lt
    ...
    loop
      ...
      values
        (p_nrolote, i, i, null, fin.vlrdespesas, 2050000, 30201100,
         ...
         case when fin.recdesp = 1 then 'Valor a compensar' else 'Comissão Integrado' end, 'COM');
    end loop;
  exception
    when others then
      p_errmsg := sqlerrm;
    ...
    loop
      ...
      begin
        insert into ad_tsffcinf
          (nufcinf, numlote, codemp, nunota, nufin, numnota, serienota, dtneg, dtfatur, vlrnota,
           codtipoper, tipmov, statusnota, statusnfe, qtdneg)
        values
          (x, notas(x).numlote, notas(x).codemp, notas(x).nunota, notas(x).nufin, notas(x).numnota,
           notas(x).serienota, notas(x).dtneg, notas(x).dtfatur, notas(x).vlrnota,
           notas(x).codtipoper, notas(x).tipmov, notas(x).statusnota, notas(x).statusnfe,
           notas(x).qtdneg);
      exception
        when others then
    ...
    begin
      ...
      values
        (l.numlote, 1, 'LR', conf.percmortprev, l.qtdaves * (conf.percmortprev / 100),
         ...
         l.qtdmortes, l.qtdaves - l.qtdmortes, 0, 0, l.viabilidade, 100 - l.viabilidade, l.percom,
         l.vlrcom, round(l.vlrcom / l.qtdabat, 3), 0, 0, 'NNA');
    exception
      when others then
  ...
  begin
    ...
    loop
      ...
      b(m).viabilidade := round(greatest((l.qtdabat / (l.qtdaves - b(m).qtdavesbnf)) * 100, 2), 2);
      b(m).percmortlote := 100 - b(m).viabilidade;
      l.ipsulote := round((b(m).viabilidade * 0.1) * (l.ganholote * 0.35) / (l.calote * 0.55), 2);
      b(m).perccom := round(ad_pkg_fci.get_perc_com(l.ipsulote, l.ipsumedio), 2);
      b(m).vlrcom := l.peso * (b(m).perccom / 100) * l.vlrunit;
      b(m).vlrunitcom := round(b(m).vlrcom / l.qtdabat, 3);
      b(m).vlrbonific := case
      ...
      b(m).vlrunitbnf := round(b(m).vlrbonific / l.qtdabat, 3);
      b(m).aprovado := 'N';
      b(m).obs := v_sem;
  ...
  begin
    set_bnf_carcaca(p_nrolote, null,p_errmsg); 
  end;
  
  procedure set_bnf_carcaca(p_nrolote number, p_vlrmedcom float, p_errmsg out varchar2) as
    v_sexo      varchar2(1);
    v_codprod   number;
    v_vlrmedcom float := 0;
    i           integer := 0;
  ...
  begin
    ...
    begin
      ...
      values
        (l.numlote, i, 'C', c.percmortprev, l.qtdaves * (c.percmortprev / 100),
         ...
         l.qtdmortes, l.qtdaves - l.qtdmortes, 0, 0, l.viabilidade, 100 - l.viabilidade, l.percom,
         l.vlrcom, l.vlrcom / l.qtdabat, 0, 0, 'N');
    exception
      when others then
        p_errmsg := 'Erro ao inserir os dados do lote real. ' || sqlerrm;
        rollback;
    ...
    begin
      ...
      if v_sexo = 'F' then
        v_codprod := ad_pkg_fci.c_prodfemeabnf;
      elsif v_sexo = 'M' then
        v_codprod := ad_pkg_fci.c_prodmachobnf;
      else
        v_codprod := ad_pkg_fci.c_prodsexadobnf;
      end if;
    
      i := 0;
      
      if nvl(p_vlrmedcom,0) = 0 then
      ...
      /*for med in (select cab.codtipoper, cab.nunota, cab.dtfatur, cab.vlrnota, ite.codprod,
        ad_get.descrproduto(ite.codprod) prod, ite.qtdneg, ite.vlrtot
                   ...
                   where 1 = 1
                     ...
                     and cab.codtipoper = 401
                     and ite.codprod = v_codprod
                     And cab.dtfatur < l.dtsaida
                   order by cab.dtfatur desc)
      loop
        if i < c.permedvlrcom then
          v_vlrmedcom := v_vlrmedcom + med.vlrtot;
          i           := i + 1;
        Else
         Exit;
        end if;
      ...
      v_vlrmedcom := v_vlrmedcom / i;*/
      
      select sum(ite.vlrtot * ite.vlrunit) / sum(ite.vlrunit)
       into v_vlrmedcom
      from tgfcab cab
                    join tgfite ite
                      on ite.nunota = cab.nunota
                   where 1 = 1
                     ...
                     and ite.codprod = v_codprod
                     And cab.dtfatur < l.dtsaida
                     and rownum <= c.permedvlrcom;
      else
      
        v_vlrmedcom := p_vlrmedcom;
    ...
    begin
      update ad_tsffcibnf bnf
         set bnf.vlrbonific = greatest(v_vlrmedcom - l.vlrcom, 0),
             bnf.vlrunitcom = bnf.vlrcom / l.qtdabat,
             bnf.obs        = case when p_vlrmedcom is null then
                                'Foram analisados ' || c.permedvlrcom || ' lotes. Média: ' ||
                                fmt.valor_moeda(v_vlrmedcom)
                              else
                               'Valor médio de '||fmt.valor_moeda(v_vlrmedcom)||' informado '||
  ...
  begin
    ...
    for lote in (select distinct l.numlfv, l.codparc, l.codprod, l.qtdaves, l.dtalojamento,
                                 l.qtdmortes, l.pesofinal, l.dtabate, l.dhpega, l.dhracao, l.gta,
                                 l.numlote, tl.nro_lote
                   from ad_tsflfv l
