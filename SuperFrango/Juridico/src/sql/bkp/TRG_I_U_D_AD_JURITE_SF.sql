Create Or Replace Trigger "TRG_I_U_D_AD_JURITE_SF"
	Before Insert Or Update Or Delete On Ad_Jurite
	For Each Row
...
Begin
  ...
  */
	...
	If :New.Situacao Not In ('P') And
		 (Updating('CODEMP') Or Updating('CODPARC') Or Updating('AGENCIA') Or Updating('CODCENCUS') Or
	...
	If (Inserting Or Updating('FORMA') Or Updating('SEQUENCIA') Or Updating('FAVORECIDO') Or
		 Updating('CPF') Or Updating('CODBCO') Or Updating('AGENCIA') Or Updating('CONTA')) And
		 :New.Forma = 'D' And :New.Sequencia Is Null And
		 (:New.Favorecido Is Null Or :New.Cpf Is Null Or :New.Codbco Is Null Or :New.Agencia Is Null Or
		 :New.Conta Is Null) Then
		Raise_Application_Error(-20101,
  ...
  */
	...
	If updating('NUMPROCESSO') Then
		...
		:new.Numprocesso := Substr(Lpad(Replace(Replace(Ltrim(Rtrim(Regexp_Replace(:new.numprocesso,
																																							 '[^0-9]+',
																																							 Null))),
																										'-',
																										''),
