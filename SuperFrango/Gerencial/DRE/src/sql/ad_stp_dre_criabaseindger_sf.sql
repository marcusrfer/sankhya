create or replace procedure ad_stp_dre_criabaseindger_sf(p_codusu    number,
                                                         p_idsessao  varchar2,
                                                         p_qtdlinhas number,
...
begin
  ...
  vt := to_char(to_date(ad_pkg_var.end_time, 'sssss'), 'hh24:mi:ss');

  if errmsg is null then
    p_mensagem := 'Base de dados recriada com sucesso' || ' em ' || vt;
  else
    p_mensagem := errmsg;
  end if;
