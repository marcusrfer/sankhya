...
Begin
   ...
   */
   ...
   Loop
      ...
      Select Count(Distinct codregfre) Into t From ad_tsfrocp Where numrocc = r_roc.numrocc;
      ...
      Select Count(*) Into t From ad_tsfrocp Where numrocc = r_roc.numrocc;
      ...
      For c In (Select * From ad_tsfrocc Where numrocc = r_roc.numrocc)
      Loop
         ...
         Begin
            ...
            Loop
               ...
               If validacao Then
                  Null;
               Else
                  Update tgfcab Set ordemcarga = dif.ordemcarga Where nunota = dif.nunota;
               End If;
            End Loop;
         ...
         Else
            ...
            Begin
               Select v.ad_codcat Into v_codcat From tgfvei v Where v.codveiculo = c.Codveiculo;
            
               If v_codcat = 0 Then
                  p_mensagem := 'Categoria do veículo não informada.<br>' || 'Veículo: <a href="' ||
                                ad_fnc_urlskw('TGFVEI', c.Codveiculo) || '" target="_parent">' || '<font color="#FF0000"><b>' ||
                                c.Codveiculo || '</b></font></a>';
                  Return;
               End If;
            Exception
               When Others Then
            ...
            Exception
               When no_data_found Then
                  p_mensagem := 'Não existe preço cadastrado para essa categoria (' || v_CodCat || ') nessa região (' ||
                                v_codregfre || ').<br>' ||
         ...
         Using (Select c.Codemp As codemp, c.Ordemcarga As ordemcarga, c.Distrota As distancia, v_Vlrrota As vlrrota From dual) D
         On (v.codemp = d.codemp And v.ordemcarga = d.ordemcarga)
         When Matched Then
            Update Set v.distrota = d.distancia, v.vlrrota = d.vlrrota
         When Not Matched Then
            Insert (v.codemp, v.ordemcarga, v.distrota, v.vlrrota) Values (d.codemp, d.ordemcarga, d.distancia, d.vlrrota);
      
         Begin
            Update ad_tsfrocc Set vlrrota = v_VlrRota Where numrocc = c.numrocc;
         Exception
            When Others Then
         ...
         For p In (
                   ...
                   Select numrocp,
                     ...
                     From (Select roc.codparctransp,
                                   ...
                                   poc.sequencia,
                                   poc.peso,
                                   (Select Sum(peso) From ad_tsfrocp p2 Where p2.numrocc = poc.numrocc) As pesototal,
                                   roc.vlrrota
                              From ad_tsfrocp poc
                              Join ad_tsfrocc roc
         ...
         Loop
            Begin
               Dbms_Output.Put_Line(p.nunota || ' | ' || p.peso || ' | ' || p.sequencia || ' | ' || p.perc || ' | ' ||
                                    p.vlrfrete);
