...
begin
  ...
  */
  ...
  if inserting then
    ...
    if :old.status != 'P' and stp_get_codusulogado not in (:old.codusulib, 0, :old.codmonitor) then
      raise_application_error(-20105,
                              ad_fnc_formataerro('Solicitações "Aprovadas" não podem ser alteradas!'));
    end if;
  
    if updating('CODSALA') or updating('DTRESERVA') or updating('HRINI') or updating('HRFIN') then
      ...
      if nvl(:new.codmonitor, :old.codmonitor) is null then
        ...
        mail.email := ad_get.mailusu(:new.codususol) || ', ' || ad_get.mailusu(:new.codmonitor);
      end if;
    
      mail.mensagem := 'Venho por meio deste informar que houve alterações na sua solicitação de reserva.' ||
                       '<br><b>Sala:</b>' || :new.codsala || ' - ' ||
                       ad_pkg_ssc.get_nomesala(:new.codsala) || '<br><b>Dt. Reserva: </b>' ||
                       :new.dtreserva || ' <b>Horário:</b> ' || fmt.hora(:new.hrini) || ' ~ ' ||
                       fmt.hora(:new.hrfin) || '<br> Alterações realizadas por ' ||
                       initcap(ad_get.nomeusu(stp_get_codusulogado, 'completo')) || ' as ' ||
