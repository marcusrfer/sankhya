...
begin
 ...
 if updating then
  ...
  if (:old.status = :new.status) and :new.status = 'conf' then
  ...
  if :old.status = 'conc' and :new.status = 'conc' then
  ...
  if (:old.statuscar != :new.statuscar) and :new.statuscar in ('E', 'P', 'AL') then
   :new.status := 'aconf';
  end if;
 
  if (:old.statuscar != :new.statuscar and :old.dhagendcarro != :new.dhagendcarro) and
  ...
  if :new.tipovisita = 'C' and :new.dhvisita is not null and :new.matfunc is not null then
   begin
    insert into ad_tsfprgvis
     (matfunc, codemp, codlot, codcargo, codcid, coduf, dtultvis, ativo)
    values
     (:new.matfunc, :new.codemp, :new.codlot, :new.codcargo, :new.codcid, :new.coduf, :new.dhvisita,
      'S');
   exception
  ...
  if :new.tipovisita = 'M' and :new.dhvisita is not null and :new.matfunc is not null then
   begin
