...
begin
 ...
 begin
  ...
  select m.serienota, m.codvend, m.codtipvenda, m.codcencus, m.codnat, m.obspadrao,
         nvl(m.confauto, 'N'), m.codprod, m.codvol, m.codlocal, m.codtipoper
    into cab.serienota, cab.codvend, cab.codtipvenda, cab.codcencus, cab.codnat,
         cab.observacao, cab.confirmnotafat, ite.codprod, ite.codvol, ite.codlocalorig,
         cab.codtipoper
    from ad_tsfmgn m
  ...
  begin
   ...
   exception
    when others then
     p_mensagem := 'Erro ao atualizar dados no item do pedido/nota. ' || sqlerrm;
     return;
   end;
  ...
  begin
   ...
   values
    (lote.numlote, i, lote.codemp, cab.nunota, v_nufin, cab.numnota, cab.serienota,
     cab.dtneg, cab.dtneg, cab.vlrnota, cab.codtipoper, top.tipmov, 'A', cab.statusnfe,
     ite.qtdneg);
  exception
   when others then
